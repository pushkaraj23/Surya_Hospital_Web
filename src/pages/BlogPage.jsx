...
import { useEffect, useState } from "react";
import { getBlogs , getFullImageUrl} from "../api/userApi";
import BlogsCard from "../components/BlogsPage/BlogCard";
import { motion } from "framer-motion";
    ...
    async function loadArticles() {
      try {
        ...
        const formatted = blogs
          .filter((blog) => blog.isactive !== false) // Only active blogs
          .map((blog) => {
            // Get plain text version for excerpt
            const plainTextContent = stripHtmlTags(blog.content);
            ...
            return {
              ...
              slug: blog.slug,
              image: getFullImageUrl(blog.image), // ✅ Transform image URL
    ...
    if (searchTerm) {
      ...
      result = result.filter(
        (a) =>
          ...
          a.plainTextContent?.toLowerCase().includes(search)
      );
    }
  ...
  if (loading)
    return (
      ...
      >
        ...
        <p className="text-primary text-lg">Loading Blogs...</p>
      </motion.div>
    );
